            ...
            <div className="max-h-[720px] overflow-y-auto custom-scrollbar">
              {forecasts.map(item => (
                <button
                  key={item.ingredient}
                  onClick={() => setSelectedIngredient(item)}
                  className={`w-full flex items-center justify-between px-8 py-7 transition-all border-b border-luxury-border/30 hover:bg-luxury-gold/[0.03] relative group ${activeIngredient?.ingredient === item.ingredient ? 'bg-luxury-gold/[0.05]' : ''
                    }`}
                >
